...
<div class="left-top-fixed-page-template d-xl-block d-none">
    <div class="main-menu-sidebar-wrap">
        ...
        <div class="copy-right d-lg-block d-none">
            <?php
                $copyright_text = !empty(cs_get_option('copyright_text')) ? cs_get_option('copyright_text'): esc_html__('Copyright © 2023 kidsa All Rights Reserved.','kidsa');
                $copyright_text = str_replace('{copy}','&copy;',$copyright_text);
                $copyright_text = str_replace('{year}',date('Y'),$copyright_text);
            ?>
            <p>
                <?php  echo wp_kses($copyright_text, kidsa()->kses_allowed_html(array('a'))); ?>
            </p>
        </div>
    </div>
...
<nav class="navbar navbar-area navbar-expand-lg navigation-style-01 navbar-area-2 navbar-default d-xl-none">
    <div class="container custom-container">
        <div class="responsive-mobile-menu">
            ...
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#kidsa_main_menu" aria-controls="kidsa_main_menu" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
        </div>
        <form action="<?php echo esc_url(home_url('/')) ?>" method="get" class="nav-left-search">
            <input type="text" name="s" placeholder="<?php echo esc_attr__('Search here','kidsa')?>">
        ...
        ?>
        <?php if (kidsa()->is_kidsa_core_active()) : ?>
